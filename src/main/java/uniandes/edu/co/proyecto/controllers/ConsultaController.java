...
public class ConsultaController {
    ...
    public ResponseEntity<?> getHistorico_Serializable(@PathVariable Long clienteId) {
        try {
            // Simula la ejecución que bloquea o detecta el conflicto
            List<?> resultado = consultaService.consultarHistoricoUsuario_Serializable(clienteId);
            return new ResponseEntity<>(resultado, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>("Error en consulta SERIALIZABLE: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<?> getHistorico_ReadCommitted(@PathVariable Long clienteId) {
        try {
            // Simula la ejecución que podría ver o no ver cambios concurrentes
            List<?> resultado = consultaService.consultarHistoricoUsuario_ReadCommitted(clienteId);
            return new ResponseEntity<>(resultado, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>("Error en consulta READ_COMMITTED: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<List<Object[]>> getTop20Conductores() {
        // Retorna un arreglo de Object[] que se debe mapear a un DTO en una versión más pulcra.
